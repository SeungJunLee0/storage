...
MuonAlignmentOutputXML::MuonAlignmentOutputXML(const edm::ParameterSet &iConfig)
    : m_fileName(iConfig.getParameter<std::string>("fileName")),
      ...
      m_suppressGEMEndcaps(iConfig.getUntrackedParameter<bool>("suppressGEMEndcaps", true)),   //Check               
      m_suppressGEMStations(iConfig.getUntrackedParameter<bool>("suppressGEMStations", true)),
      m_suppressGEMRings(iConfig.getUntrackedParameter<bool>("suppressGEMRings", true)),
      m_suppressGEMSuperChambers(iConfig.getUntrackedParameter<bool>("suppressGEMSuperChambers", true)),
      m_suppressGEMChambers(iConfig.getUntrackedParameter<bool>("suppressGEMChambers", false)),//eta add
      m_suppressGEMEtaPartitions(iConfig.getUntrackedParameter<bool>("suppressGEMEtaPartitions", false)),//eta add
...
void MuonAlignmentOutputXML::write(AlignableMuon *alignableMuon, const edm::EventSetup &iSetup) const {
  ...
  AlignmentErrorsExtended *gemErrors = alignableMuon->gemAlignmentErrorsExtended();//Check
  ...
  align::Alignables endcaps_GEM = alignableMuon->GEMEndcaps();//Check
  ...
  if (m_relativeto == 1) {
    ...
    align::Alignables ideal_endcaps_GEM = ideal_alignableMuon.GEMEndcaps();//Check

    writeComponents(barrels, ideal_barrels, errors, outputFile, true, false,false, alignableMuon->objectIdProvider());
    writeComponents(endcaps, ideal_endcaps, errors, outputFile, false, true,false, alignableMuon->objectIdProvider());
    writeComponents(endcaps_GEM, ideal_endcaps_GEM, errors, outputFile, false, false,true, alignableMuon->objectIdProvider());
  } else {
    align::Alignables empty1, empty2, empty3;

    writeComponents(barrels, empty1, errors, outputFile, true, false,false, alignableMuon->objectIdProvider());
    writeComponents(endcaps, empty2, errors, outputFile, false, true,false, alignableMuon->objectIdProvider());
    writeComponents(endcaps_GEM, empty3, errors, outputFile, false, false,true, alignableMuon->objectIdProvider());
  }
...
//problem!!!!!!!
    if ((alignableObjectId == align::AlignableDTBarrel && !m_suppressDTBarrel) ||
        ...
        (!DT && !GEM&& CSC && alignableObjectId == align::AlignableDetUnit && !m_suppressCSCLayers) ||
        //(!DT && !GEM&& CSC && alignableObjectId == align::AlignableDetUnit && !m_suppressCSCLayers)){
        (!DT && !CSC && GEM && alignableObjectId == align::AlignableDetUnit && !m_suppressGEMSuperChambers)) {
...
//Check
      ...
      if(!CSC && !DT && GEM){  // GEM
        if (m_rawIds && rawId != 0) {
          ...
          if (alignableObjectId == align::AlignableDetUnit) {
            GEMDetId id(rawId);
            //outputFile << "  <GEMChambers endcap=\"" << id.region() << "\" station=\"" << id.station() << "\" ring=\""
            outputFile << "  <GEMSuperChambers endcap=\"" << id.region() << "\" station=\"" << id.station() << "\" ring=\""
                       << id.ring() << "\" superchamber=\"" << id.chamber() << "\" layer=\"" << id.layer() << "\" />"
                       << std::endl;
          } else if (alignableObjectId == align::AlignableGEMSuperChamber) {
            GEMDetId id(rawId);
            outputFile << "  <GEMSuperChamber endcap=\"" << id.region() << "\" station=\"" << id.station() << "\" ring=\""
                       << id.ring() << "\" chamber=\"" << id.chamber() << "\" />" << std::endl;
          } else {
      ...
      if (m_relativeto == 0) {
		...
		bool gem_debug = false;
        if (gem_debug && !DT && !CSC) {
          GEMDetId id(rawId);
          if (id.region() == 1 && id.station() == 1 && id.ring() == 1 && id.chamber() == 33) {
            std::cout << " investigating " << id << std::endl
                      << (*alignable)->globalRotation() << std::endl
                      << std::endl
                      << idealRotation.transposed() << std::endl
    ...
    if (ideal != ideals.end()) {
      ...
      writeComponents(components, ideal_components, errors, outputFile, DT, CSC,GEM, objectIdProvider);
      ++ideal;  // important for synchronization in the "for" loop!
    } else {
      align::Alignables components = (*alignable)->components();
      align::Alignables dummy;
      writeComponents(components, dummy, errors, outputFile, DT, CSC, GEM, objectIdProvider);
    }
